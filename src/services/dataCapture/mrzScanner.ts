  ...
  try {
    ...
    return {
      ...
      birthDate: parsed.BirthDate
        ? `19${parsed.BirthDate.slice(0, 2)}-${parsed.BirthDate.slice(
            2,
            4
          )}-${parsed.BirthDate.slice(4, 6)}`
        : null,
      expiryDate: parsed.ExpiryDate
        ? `20${parsed.ExpiryDate.slice(0, 2)}-${parsed.ExpiryDate.slice(
            2,
            4
          )}-${parsed.ExpiryDate.slice(4, 6)}`
        : null,
