...
import ScanResultModal, {
  ScanResultData,
} from "../components/modals/ScanResultModal";
...
export default function DataCapture() {
  ...
  function openResult(
    title: string,
    section: string,
    data: ScanResultData | null | undefined
  ) {
    if (!data) return;
    setScanData(data);
    setModalTitle(title);
  ...
  return (
    <View>
      ...
      >
        ...
        <ModuleCard
          title="MRZ Scanner"
          description="Scan Machine Readable Zones from passports and IDs"
          icon="scan-outline"
          onPress={handleMrzScan}
        />
        <ModuleCard
          title="US Driver's License Scanner"
          description="Scan US driver's licenses and extract structured data"
          icon="card-outline"
          onPress={handleDriverLicenseScan}
        />
        <ModuleCard
          title="German ID Scanner"
          description="Scan German identity cards and capture personal data"
          icon="person-outline"
          onPress={handleGermanIdScan}
        />
        <ModuleCard
          title="EHIC Scanner"
          description="Scan European Health Insurance Cards"
          icon="medkit-outline"
          onPress={handleEhicScan}
        />
        <Text
          style={{
            paddingBottom: 20,
        ...
        <ModuleCard
          title="Check Scanner"
          description="Extract check data from the MICR code on the bottom of a check"
          icon="cash-outline"
          onPress={handleCheckScan}
        />
        <ModuleCard
          title="Credit Card Scanner"
          description="Extract details from the front side of Visa, MasterCard and AMEX cards"
          icon="card-outline"
          onPress={handleCreditCardScan}
        />

        <ModuleCard
          title="POC Credit Card"
          description="Extract details from the front side of Visa, MasterCard and AMEX cards"
          icon="card-outline"
          onPress={startCreditCardPOC}
        />
        ...
        <ModuleCard
          title="Data Scanner"
          description="Capture single line data in an instant"
          icon="text-outline"
          onPress={handleDataScanner}
        />
        <ModuleCard
          title="VIN Scanner"
          description="Extract Vehicle Identification Numbers located behind the windshield or in the driver's door."
          icon="car-outline"
