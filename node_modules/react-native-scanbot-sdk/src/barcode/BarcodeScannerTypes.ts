...
} from '../barcode/BarcodeConfigurationTypes';
import { BarcodeDocumentFormat } from '../barcode/BarcodeDocumentTypes';
import { BarcodeFormat, RangeEncoding, StructuredAppendInfo } from '../barcode/BarcodeTypes';
...
export class BarcodeItem extends PartiallyConstructible {
  ...
  /**
    ...
    */
  public readonly rawBytesEncodings: RangeEncoding[];
  /**
  ...
  /**
    ...
    Default is 0
    */
  public readonly sizeScore: number = 0.0;
  /**
    The parsed known document format (if parsed successfully).
    */
  public readonly extractedDocument: GenericDocument | null;
  /**
    ...
    Default is -1
    */
  public readonly globalIndex: number = -1;
  ...
  public constructor(source: DeepPartial<BarcodeItem> = {}) {
    ...
    if (source.rawBytesEncodings !== undefined) {
      this.rawBytesEncodings = source.rawBytesEncodings.map((it: any) => {
    ...
    if (source.structuredAppendInfo !== undefined) {
      this.structuredAppendInfo =
        source.structuredAppendInfo != null
          ? new StructuredAppendInfo(source.structuredAppendInfo)
          : null;
    } else {
    ...
    if (source.quadNormalized !== undefined) {
      this.quadNormalized = source.quadNormalized.map((it: any) => {
        return { x: it.x, y: it.y };
      });
    } else {
    ...
    if (source.extendedQuad !== undefined) {
      this.extendedQuad = source.extendedQuad.map((it: any) => {
        return { x: it.x, y: it.y };
      });
    } else {
    ...
    if (source.extendedQuadNormalized !== undefined) {
      this.extendedQuadNormalized = source.extendedQuadNormalized.map((it: any) => {
        return { x: it.x, y: it.y };
      });
    } else {
    ...
    if (source.extractedDocument !== undefined) {
      this.extractedDocument =
        source.extractedDocument != null ? new GenericDocument(source.extractedDocument) : null;
    } else {
  ...
  public async serialize(
    ...
    return {
      ...
      quadNormalized: this.quadNormalized.map((it: any) => {
        return it;
      }),
      extendedQuad: this.extendedQuad.map((it: any) => {
        return it;
      }),
      extendedQuadNormalized: this.extendedQuadNormalized.map((it: any) => {
        return it;
      }),
      ...
      extractedDocument:
        this.extractedDocument != null ? await this.extractedDocument.serialize(config) : null,
...
export const BarcodeScannerEngineModeValues: BarcodeScannerEngineMode[] = [
  'LEGACY',
  'NEXT_GEN_LOW_POWER',
  'NEXT_GEN',
  'NEXT_GEN_LOW_POWER_FAR_DISTANCE',
...
export const BarcodeAccumulationMethodValues: BarcodeAccumulationMethod[] = [
  'LAST_VISIBLE',
  'INTERPOLATE_BY_CAMERA',
];
...
export class BarcodeAccumulationConfiguration extends PartiallyConstructible {
  /**
    ...
    Default is 500
    */
  public accumulationTime: number = 500;
  /**
    ...
    Default is true
    */
  public removeUnconnectedResults: boolean = true;
  /**
    The method how to display barcodes from previous frames.
    
    Default is INTERPOLATE_BY_CAMERA
    */
  public method: BarcodeAccumulationMethod = 'INTERPOLATE_BY_CAMERA';
...
export class BarcodeScannerConfiguration extends PartiallyConstructible {
  /**
    ...
    */
  public barcodeFormatConfigurations: BarcodeFormatConfigurationBase[] = [
  ...
  /**
    ...
    */
  public extractedDocumentFormats: BarcodeDocumentFormat[] = [
    'AAMVA',
    'BOARDING_PASS',
    'DE_MEDICAL_PLAN',
    'MEDICAL_CERTIFICATE',
  ...
  /**
    ...
    Default is false
    */
  public onlyAcceptDocuments: boolean = false;
  /**
  ...
  /**
    ...
    */
  public accumulationConfig: BarcodeAccumulationConfiguration =
  ...
  public constructor(source: DeepPartial<BarcodeScannerConfiguration> = {}) {
    ...
    if (source.barcodeFormatConfigurations !== undefined) {
      this.barcodeFormatConfigurations = source.barcodeFormatConfigurations.map((it: any) => {
        return BarcodeFormatConfigurationBase.From(it);
      });
    }
    if (source.extractedDocumentFormats !== undefined) {
      this.extractedDocumentFormats = source.extractedDocumentFormats.map((it: any) => {
        return it;
      });
    }
...
export class BarcodeScannerResult extends PartiallyConstructible {
  /**
    ...
    */
  public readonly barcodes: BarcodeItem[];
  /**
  ...
  public constructor(source: DeepPartial<BarcodeScannerResult> = {}) {
    ...
    if (source.barcodes !== undefined) {
      this.barcodes = source.barcodes.map((it: any) => {
  ...
  public async serialize(
    config: ToJsonConfiguration = new ToJsonConfiguration()
  ): Promise<DeepPartial<BarcodeScannerResult>> {
    return {
      barcodes: await Promise.all(
        this.barcodes.map(async (it: any) => {
