...
import { CommonFieldType, CommonFieldTypeValues } from '../documents/CommonFieldType';
...
export const FieldValidationStatusValues: FieldValidationStatus[] = [
  'INVALID',
  'OBSTRUCTED',
  'NONE',
  'CONFIRMED',
...
export const FieldDataFormatValues: FieldDataFormat[] = [
  'ISO_DATE',
  'ISO_COUNTRY_ALPHA_2',
  'ISO_COUNTRY_ALPHA_3',
  'ISO_COUNTRY_NUMERIC',
...
Parsed data.
*/
export class FieldParsedData extends PartiallyConstructible {
  /**
    Parsing type.
    */
  public readonly type: FieldDataFormat;
  /**
    Parsed value.
    */
  ...
  public constructor(source: DeepPartial<FieldParsedData> = {}) {
    super();
...
*/
export class FieldType extends PartiallyConstructible {
  /**
  ...
  public constructor(source: DeepPartial<FieldType> = {}) {
    super();
    ...
    if (source.commonType !== undefined) {
      this.commonType =
        source.commonType != null
          ? CommonFieldTypeValues.includes(source.commonType)
            ? source.commonType
            : null
          : null;
    } else {
...
export class Field extends PartiallyConstructible {
  ...
  /**
    ...
    Default is 1.0
    */
  public readonly confidenceWeight: number = 1.0;
  /**
    Crop of the field.
    */
  public readonly image: ImageRef | null = null;
  /**
  ...
  /**
    ...
    Default is NONE
    */
  public readonly validationStatus: FieldValidationStatus = 'NONE';
  /**
    Parsed data.
    */
  public readonly parsedData: FieldParsedData[] = [];
  ...
  public constructor(source: DeepPartial<Field> = {}) {
    ...
    if (source.image !== undefined) {
      this.image = source.image != null ? ImageRef.From(source.image) : null;
    }
    if (source.polygonInRoot !== undefined) {
      this.polygonInRoot = source.polygonInRoot.map((it: any) => {
    ...
    if (source.parsedData !== undefined) {
      this.parsedData = source.parsedData.map((it: any) => {
  ...
  public async serialize(
    ...
    return {
      ...
      image: config.serializeImages
        ? this.image != null
          ? await this.image.serialize(config.imageSerializationMode)
          : null
        : undefined,
      polygonInRoot: this.polygonInRoot.map((it: any) => {
        return it;
      }),
      validationStatus: this.validationStatus,
      parsedData: this.parsedData.map((it: any) => {
...
Generic Document Type.
*/
export class GenericDocumentType extends PartiallyConstructible {
  /**
  ...
  public constructor(source: DeepPartial<GenericDocumentType> = {}) {
    super();
    if (source.name !== undefined) {
      this.name = source.name;
    } else {
  ...
  public serialize(
    config: ToJsonConfiguration = new ToJsonConfiguration()
  ): DeepPartial<GenericDocumentType> {
    return {
      name: this.name,
      fullName: this.fullName,
      normalizedName: this.normalizedName,
...
export class GenericDocument extends PartiallyConstructible {
  /**
    Document type.
    */
  public readonly type: GenericDocumentType;
  /**
  ...
  /**
    ...
    */
  public readonly quadInRoot: Point[];
  /**
    Image crop of the document.
    */
  public readonly crop: ImageRef | null = null;
  /**
  ...
  public constructor(source: DeepPartial<GenericDocument> = {}) {
    ...
    if (source.children !== undefined) {
      this.children = source.children.map((it: any) => {
    ...
    if (source.quadInRoot !== undefined) {
      this.quadInRoot = source.quadInRoot.map((it: any) => {
        return { x: it.x, y: it.y };
      });
    } else {
    ...
    if (source.crop !== undefined) {
      this.crop = source.crop != null ? ImageRef.From(source.crop) : null;
    }
    if (source.confidence !== undefined) {
      this.confidence = source.confidence;
    }
  ...
  public async serialize(
    config: ToJsonConfiguration = new ToJsonConfiguration()
  ): Promise<DeepPartial<GenericDocument>> {
    return {
      type: this.type.serialize(config),
      fields: await Promise.all(
        this.fields.map(async (it: any) => {
          return await it.serialize(config);
        })
      ),
      children: await Promise.all(
        this.children.map(async (it: any) => {
          return await it.serialize(config);
        })
      ),
      ...
      quadInRoot: this.quadInRoot.map((it: any) => {
        return it;
      }),
      crop: config.serializeImages
        ? this.crop != null
          ? await this.crop.serialize(config.imageSerializationMode)
          : null
        : undefined,
      confidence: this.confidence,
      confidenceWeight: this.confidenceWeight,
  ...
  public async encodeImages(): Promise<void> {
    await Promise.all(
      this.fields.map(async (it) => {
        await it.encodeImages();
      })
    );
    await Promise.all(
      this.children.map(async (it) => {
        await it.encodeImages();
      })
    );
    if (this.crop != null) {
