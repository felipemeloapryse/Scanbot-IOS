...
fun ReadableArray.extractBarcodeFormatConfigurations(): List<BarcodeFormatConfigurationBase> {

    val formatConfigs = mutableListOf<BarcodeFormatConfigurationBase>()

    for (index in 0..<size()) {
        getMap(index)?.toHashMap()?.let {
            formatConfigs.add(BarcodeFormatConfigurationBase(it))
        }
    }
