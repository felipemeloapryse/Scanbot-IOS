...
exports.BarcodeScannerResult = exports.BarcodeScannerEngineModeValues = exports.BarcodeScannerConfiguration = exports.BarcodeItem = exports.BarcodeAccumulationMethodValues = exports.BarcodeAccumulationConfiguration = void 0;
var _BarcodeConfigurationTypes = require("../barcode/BarcodeConfigurationTypes");
var _BarcodeTypes = require("../barcode/BarcodeTypes");
...
class BarcodeItem extends _utils.PartiallyConstructible {
  ...
  constructor(source = {}) {
    ...
    if (source.rawBytesEncodings !== undefined) {
      this.rawBytesEncodings = source.rawBytesEncodings.map(it => {
        return new _BarcodeTypes.RangeEncoding(it);
      });
    } else {
      throw new Error('rawBytesEncodings must be present in constructor argument');
    }
    if (source.structuredAppendInfo !== undefined) {
      this.structuredAppendInfo = source.structuredAppendInfo != null ? new _BarcodeTypes.StructuredAppendInfo(source.structuredAppendInfo) : null;
    } else {
    ...
    if (source.sourceImage !== undefined) {
      this.sourceImage = source.sourceImage != null ? _image.ImageRef.From(source.sourceImage) : null;
    }
    ...
    if (source.extractedDocument !== undefined) {
      this.extractedDocument = source.extractedDocument != null ? new _GenericDocument.GenericDocument(source.extractedDocument) : null;
    } else {
  ...
  async serialize(config = new _JsonSerializationTypes.ToJsonConfiguration()) {
    return {
      ...
      sourceImage: config.serializeImages ? this.sourceImage != null ? await this.sourceImage.serialize(config.imageSerializationMode) : null : undefined,
...
const BarcodeScannerEngineModeValues = exports.BarcodeScannerEngineModeValues = ['LEGACY', 'NEXT_GEN_LOW_POWER', 'NEXT_GEN', 'NEXT_GEN_LOW_POWER_FAR_DISTANCE', 'NEXT_GEN_FAR_DISTANCE'];
...
const BarcodeAccumulationMethodValues = exports.BarcodeAccumulationMethodValues = ['LAST_VISIBLE', 'INTERPOLATE_BY_CAMERA'];

/**
Configuration for how to accumulate results.
*/
class BarcodeAccumulationConfiguration extends _utils.PartiallyConstructible {
  /**
  ...
  /**
    ...
    Default is true
    */
  removeUnconnectedResults = true;
  /**
    The method how to display barcodes from previous frames.
    
    Default is INTERPOLATE_BY_CAMERA
    */
  method = 'INTERPOLATE_BY_CAMERA';
...
*/
exports.BarcodeAccumulationConfiguration = BarcodeAccumulationConfiguration;
class BarcodeScannerConfiguration extends _utils.PartiallyConstructible {
  /**
    Options for barcode decoding.
    */
  barcodeFormatConfigurations = [new _BarcodeConfigurationTypes.BarcodeFormatCommonConfiguration({})];
  /**
    ...
    */
  extractedDocumentFormats = ['AAMVA', 'BOARDING_PASS', 'DE_MEDICAL_PLAN', 'MEDICAL_CERTIFICATE', 'ID_CARD_PDF_417', 'SEPA', 'SWISS_QR', 'VCARD', 'GS1', 'HIBC'];
  /**
  ...
  constructor(source = {}) {
    ...
    if (source.barcodeFormatConfigurations !== undefined) {
      this.barcodeFormatConfigurations = source.barcodeFormatConfigurations.map(it => {
        return _BarcodeConfigurationTypes.BarcodeFormatConfigurationBase.From(it);
      });
    }
...
*/
exports.BarcodeScannerConfiguration = BarcodeScannerConfiguration;
class BarcodeScannerResult extends _utils.PartiallyConstructible {
  /**
...
exports.BarcodeScannerResult = BarcodeScannerResult;
